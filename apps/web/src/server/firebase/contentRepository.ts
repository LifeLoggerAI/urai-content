...
function collection(db: FirestoreLike, path: string): CollectionReferenceLike {
  return db.collection(path);
}

function asRecord<T>(value: T): FirestoreData {
  return value as FirestoreData;
}

function asType<T>(value: FirestoreData): T {
  return value as T;
}
...
export function createFirestoreContentRepository(db: FirestoreLike): ContentRepository {
  return {
    async upsertContent(item: ContentItem): Promise<void> {
      await collection(db, FIRESTORE_COLLECTIONS.contentItems).doc(item.id).set(asRecord(item), { merge: true });
    },
    ...
    async deleteContent(id: string): Promise<void> {
      await collection(db, FIRESTORE_COLLECTIONS.contentItems).doc(id).delete();
    },
    ...
    async addVersion(contentId: string, snapshot: ContentItem): Promise<number> {
      ...
      await collection(db, FIRESTORE_COLLECTIONS.contentVersions)
        .doc(`${contentId}-v${version}`)
        .set(asRecord({ contentId, version, snapshot }));
      return version;
    },
    ...
    async logModeration(item: ModerationQueueItem): Promise<void> {
      await collection(db, FIRESTORE_COLLECTIONS.moderationQueue).doc(item.id).set(asRecord(item));
    },

    async logRelease(release: PublishingRelease): Promise<void> {
      await collection(db, FIRESTORE_COLLECTIONS.publishingReleases).doc(release.id).set(asRecord(release));
    },

    async addTelemetry(event: TelemetryEvent): Promise<void> {
      await collection(db, FIRESTORE_COLLECTIONS.telemetryEvents).add(asRecord(event));
    },
    ...
    async upsertNarratorPrompt(prompt: NarratorPrompt): Promise<void> {
      await collection(db, FIRESTORE_COLLECTIONS.narratorPrompts).doc(prompt.id).set(asRecord(prompt), { merge: true });
    },

    async upsertStoryTemplate(template: StoryTemplate): Promise<void> {
      await collection(db, FIRESTORE_COLLECTIONS.storyTemplates).doc(template.id).set(asRecord(template), { merge: true });
    },

    async upsertRitualTemplate(template: RitualTemplate): Promise<void> {
      await collection(db, FIRESTORE_COLLECTIONS.ritualTemplates).doc(template.id).set(asRecord(template), { merge: true });
    },

    async upsertMarketplaceItem(item: MarketplaceItem): Promise<void> {
      await collection(db, FIRESTORE_COLLECTIONS.marketplaceItems).doc(item.id).set(asRecord(item), { merge: true });
    },

    async upsertCreatorSubmission(item: CreatorSubmission): Promise<void> {
      await collection(db, FIRESTORE_COLLECTIONS.creatorSubmissions).doc(item.id).set(asRecord(item), { merge: true });
    },

    async upsertExportTemplate(item: ExportTemplate): Promise<void> {
      await collection(db, FIRESTORE_COLLECTIONS.exportTemplates).doc(item.id).set(asRecord(item), { merge: true });
    }
  };
}
