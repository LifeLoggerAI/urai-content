...
const allowedNegations = ['does not sell your private data', "doesn't sell your private data"]; 
  ...
  for (const file of files) {
    ...
    for (const item of items) {
      ...
      if (parsed.visibility === 'public') {
        ...
        if (unsafe && !(unsafe === 'sell your private data' && hasAllowedNegation)) throw new Error(`Unsafe public claim '${unsafe}' in ${relative(root, file)}`);
      }
